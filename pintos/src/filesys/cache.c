...
static void
cache_block_init (struct cache_block *c_b)
{
...
static void
cache_write_back (struct block *fs_device, struct cache_block *cb)
{
...
void
cache_read (struct block *fs_device, block_sector_t sector, void *dst, off_t offset, int chunk_size)
{
...
void
cache_write (struct block *fs_device, block_sector_t sector, void *src, off_t offset, int chunk_size)
{
    ASSERT (offset < BLOCK_SECTOR_SIZE);
